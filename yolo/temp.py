...
while True:
    ...
    # YOLOv5 결과에서 bbox와 confidence, class id 추출
    bbox_xywh = []
    confs = []
    clss = []

    for *xyxy, conf, cls in results.pred[0]:
        # YOLOv5의 결과는 [x1, y1, x2, y2] 형태이므로, [x_center, y_center, width, height]로 변환
        x1, y1, x2, y2 = xyxy
        x_c, y_c, bbox_w, bbox_h = (x1 + x2) / 2, (y1 + y2) / 2, x2 - x1, y2 - y1
        bbox_xywh.append([x_c, y_c, bbox_w, bbox_h])
        confs.append(conf.item())
        clss.append(int(cls.item()))  # Ensure class IDs are integers

    # Convert lists to tensors
    bbox_xywh = torch.tensor(bbox_xywh, dtype=torch.float32)  # Ensure float32 dtype
    confs = torch.tensor(confs, dtype=torch.float32)  # Ensure float32 dtype

    # DeepSORT로 객체 추적
    # Convert tensors to lists as required by DeepSORT
    bbox_xywh = bbox_xywh.tolist()
    confs = confs.tolist()
    
    # Ensure bbox_xywh is in the format [x_center, y_center, width, height]
    detections = [(bbox_xywh[i], confs[i]) for i in range(len(bbox_xywh))]
